...
- 月度预算：按分类设置上限，达到 90% 或超支时进入资金提醒与邮件摘要
- 周期交易：房租/订阅等固定收支按月/周由后台任务自动生成
...
- 借款计息：借出/借入可选年化利率，按实际持有天数显示应计利息，结清时自动记入利息收入或支出；有息借款需一次性结清
...
- 资金提醒：聚合未来 N 天内到期的定存、借款、未还信用卡账单、固定账单、未完成储蓄目标与当月预算预警；可选 SMTP 邮件摘要
- 净资产趋势：按日期和显示币种保存每日快照，总览展示最近一年趋势；旧快照不会因修改历史交易而回溯重算
...
| `KOKU_JOBS_INTERVAL_MINUTES` | `60` | 服务端周期交易、预算结转、净资产每日快照与收盘后行情刷新检查间隔（分钟，1–1440） |
...
| `GET` | `/api/summary/net-worth-trend` | 查询最近 `?days=`（默认 365，上限 3650）天的净资产每日快照；读取时会先刷新今天的快照 |
