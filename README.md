...
- 资产与负债账户；同一账户可维护 CNY、USD 等多个独立币种余额
- 收入、支出、同币种与跨币种转账
- SQLite 原子余额更新与账单撤销
...
账户响应中的 `balances` 按币种列出独立余额。创建收入/支出时通过 `currency` 指定本笔交易币种；转账通过 `source_currency` 与 `target_currency` 分别指定两端币种，因此同一账户也可以完成 CNY → USD 的内部换汇。
