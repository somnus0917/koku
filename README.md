...
- 信用卡账单（v1）：信用账户可设账单日/还款日（1~31，无对应日期自动落到月末），账户页展示额度/已用/可用、本期已出账与未出账金额、下次账单与还款日；消费 = Credit 账户上的支出，还款 = 储蓄 → 信用卡的转账（**还款不是支出**，不会重复统计）；已用额度以账户实际余额为准（`max(0, balance)`，期初余额/余额调整/退款一并计入）；出账/未出账按 `occurred_at` 与账单日动态划分（无快照表），还款/贷项按 FIFO 冲抵最早消费（可解释近似，不追踪「某次还款具体还哪一期」）；金额全程 `Decimal` 精确求和，不使用浮点。注：v1 暂不提供信用卡跨账期逾期提醒（动态账单无快照，跨期后提醒不可靠），待引入正式账单快照/结算模型后再恢复
