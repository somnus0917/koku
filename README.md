...
- 信用卡账单（v1）：信用账户可设账单日/还款日（1~31，无对应日期自动落到月末），账户页展示额度/已用/可用、本期已出账与未出账金额、下次账单与还款日；消费 = Credit 账户上的支出，还款 = 储蓄 → 信用卡的转账（**还款不是支出**，不会重复统计）；出账/未出账按 `occurred_at` 与账单日动态划分（无快照表），还款按 FIFO 冲抵最早消费（可解释近似，不追踪「某次还款具体还哪一期」）；信用卡还款日进入既有到期提醒
